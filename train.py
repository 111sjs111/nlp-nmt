...
def train_model(config):
    ...
    for epoch in range(config.epochs):
        ...
        logging.info(msg)
        if epoch % 1 == 0:  
            bleu = evaluate(config, valid_iter, translation_model, data_loader)
            logging.info(f"BLEU on validation: {bleu:.3f}")
...
def evaluate(config, valid_iter, model, data_loader):
    ...
    with torch.no_grad():
        for idx, (src, tgt) in enumerate(valid_iter):
            ...
            output = logits.argmax(dim=-1)  # [tgt_len, batch_size]
            output = output.permute(1, 0).tolist()
            tgt_out = tgt[1:, :].permute(1, 0).tolist()

            PAD_IDX = data_loader.PAD_IDX
            vocab = data_loader.en_vocab

            for pred_ids, ref_ids in zip(output, tgt_out):
                pred_tokens = [vocab.lookup_token(tok) for tok in pred_ids if tok != PAD_IDX]
                ref_tokens = [vocab.lookup_token(tok) for tok in ref_ids if tok != PAD_IDX]

                if len(pred_tokens) > 0 and len(ref_tokens) > 0:
                    candidates_all.append(pred_tokens)
                    references_all.append([ref_tokens])  # 注意嵌套

    bleu = bleu_score(candidates_all, references_all)
    model.train()
    return bleu
